...
if GAL.nW == 1
    ...
    for in = 1:GAL.nW
        SOL{in} = compile_solution(SOL,MOD,NUM,order,GAL.Dx_x(:,:,in),GAL.Dx_z(:,:,in),GAL.Dy_x(:,:,in),GAL.Dy_z(:,:,in));
    end
    % disp('in>1')
    % for in = 1
    %     SOL = compile_solution(SOL,MOD,NUM,order,GAL.Dx_x(:,:,in),GAL.Dx_z(:,:,in),GAL.Dy_x(:,:,in),GAL.Dy_z(:,:,in));
    % end
end
