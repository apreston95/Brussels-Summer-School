...
f4 = figure;
plot(SIM.LX(47:315),SIM.Le(47:315),'LineWidth',1.5);
% xlim([0 269]);
ylabel('Hours (log deviation)');
xlabel('X (log deviation)');
...
% Plot non-stochastic sim:
f3 = figure;
plot(NSH.LX(par.sim.T_ini+1:par.sim.TT),NSH.Le(par.sim.T_ini+1:par.sim.TT),'LineWidth',1.5);
% xlim([0 269]);
ylabel('Hours (log deviation)');
xlabel('X (log deviation)');
title('Non-stochastic simulation');
